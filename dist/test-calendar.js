    ...
    try {
        ...
        try {
            ...
            for (const query of testQueries) {
                ...
                const result = await server['handleParseNaturalQuery']({ query });
        ...
        while (true) {
            ...
            if (userQuery.trim()) {
                try {
                    console.log('\n🤖 Parsing your query...');
                    const result = await server['handleParseNaturalQuery']({ query: userQuery });
                    console.log(`✅ Parsed successfully!`);
                    console.log(`🎯 Intent: ${result.intent}`);
                    console.log(`📊 Confidence: ${Math.round(result.confidence * 100)}%`);
                    if (Object.keys(result.entities).some((key) => result.entities[key])) {
